...
#define SIZE_SLICE_1_A 4
#define SIZE_SLICE_1_I 1
#define SIZE_SLICE_1_J 8
#define SIZE_SLICE_1_C 8
#define SIZE_SLICE_1_B 1

#define SIZE_INT_UNIT_1 SIZE_SLICE_1_Q

#define SIZE_TB_1_X SIZE_SLICE_1_K *SIZE_SLICE_1_I
#define SIZE_TB_1_Y SIZE_SLICE_1_J *SIZE_SLICE_1_B
#define SIZE_REG_1_X SIZE_SLICE_1_A
#define SIZE_REG_1_Y SIZE_SLICE_1_C

#define NUM_INDEX 6

// created by tc_gen_code_Kernel()
__global__ void kiaq_bcjq_abcijk_32x32x32x32x32x32x32_1(float *dev_t3, float *dev_t2, float *dev_v2,
  ...
  int idx_i = threadIdx.x / SIZE_SLICE_1_K;
  int idx_j = threadIdx.y % SIZE_SLICE_1_J;
  int idx_b = threadIdx.y / SIZE_SLICE_1_J;
  ...
  int t3_base_thread =
      blk_idx_k * SIZE_SLICE_1_K + idx_k +
      (blk_idx_j * SIZE_SLICE_1_J + idx_j +
       (blk_idx_i * SIZE_SLICE_1_I + idx_i +
        (blk_idx_c * SIZE_SLICE_1_C + (blk_idx_b * SIZE_SLICE_1_B + idx_b +
                                       (blk_idx_a * SIZE_SLICE_1_A) * size_b) *
                                          size_c) *
            size_i) *
           size_j) *
          size_k;
...
// tensor contraction: [[16, 'STR_SD2_T2_H7', 'x', 't2', ['q', 'a', 'i', 'k']],
// [16, 'STR_SD2_V2_H7', 'y', 'v2', ['q', 'j', 'c', 'b']], '+=']
#pragma unroll 1
  for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1) {
    ...
    for (int ll = 0; ll < 4; ll++) {
      ...
      // Exception: Temp. version!: 0 < rng_i
      sm_a[threadIdx.x][threadIdx.y + 0 + ll * 16] =
          dev_t2[(blk_idx_a * SIZE_SLICE_1_A + ll +
                  (blk_idx_i * SIZE_SLICE_1_I + 0 +
                   (blk_idx_k * SIZE_SLICE_1_K + idx_j + 0) * size_i) *
                      size_a) *
                     size_q +
                 (threadIdx.x + l)];
      // Exception: Temp. version!: threadIdx.x + l
      // Exception: Temp. version!: 0 < rng_i
      // Exception: Full-Full
      sm_a[threadIdx.x][threadIdx.y + 8 + ll * 16] =
          dev_t2[(blk_idx_a * SIZE_SLICE_1_A + ll +
                  (blk_idx_i * SIZE_SLICE_1_I + 0 +
                   (blk_idx_k * SIZE_SLICE_1_K + idx_j + 8) * size_i) *
                      size_a) *
                     size_q +
                 (threadIdx.x + l)];
    }
    ...
    for (int ll = 0; ll < 8; ll++) {
      // ['q', 'j', 'c', 'b']
      // Exception: Temp. version!: threadIdx.x + l
      // Exception: Temp. version!: idx_j < rng_j
      sm_b[threadIdx.x][threadIdx.y + ll * 8] =
          dev_v2[(blk_idx_j * SIZE_SLICE_1_J + idx_j +
                  (blk_idx_c * SIZE_SLICE_1_C + ll +
                   (blk_idx_b * SIZE_SLICE_1_B + 0) * size_c) *
                      size_j) *
                     size_q +
                 (threadIdx.x + l)];
    }
    ...
    for (int ll = 0; ll < SIZE_INT_UNIT_1; ll++) {
      temp_bv[0] = sm_b[ll][idx_j + (idx_b)*SIZE_SLICE_1_J + 0];
      temp_bv[1] = sm_b[ll][idx_j + (idx_b)*SIZE_SLICE_1_J + 8];
      temp_bv[2] = sm_b[ll][idx_j + (idx_b)*SIZE_SLICE_1_J + 16];
      temp_bv[3] = sm_b[ll][idx_j + (idx_b)*SIZE_SLICE_1_J + 24];
      temp_bv[4] = sm_b[ll][idx_j + (idx_b)*SIZE_SLICE_1_J + 32];
      temp_bv[5] = sm_b[ll][idx_j + (idx_b)*SIZE_SLICE_1_J + 40];
      temp_bv[6] = sm_b[ll][idx_j + (idx_b)*SIZE_SLICE_1_J + 48];
      temp_bv[7] = sm_b[ll][idx_j + (idx_b)*SIZE_SLICE_1_J + 56];

      for (int xx = 0; xx < 4; xx++) // (1)
      {
        temp_av = sm_a[ll][idx_i + (idx_k)*SIZE_SLICE_1_I + (xx * 16)];
...
// created by tc_gen_code_Kernel()
__global__ void kiaq_bcjq_abcijk_32x32x32x32x32x32x32_2(float *dev_t3, float *dev_t2, float *dev_v2,
                            int size_k, int size_j, int size_i, int size_c,
                            int size_b, int size_a, int size_q, int numBlk_k,
                            int numBlk_j, int numBlk_i, int numBlk_c,
                            int numBlk_b, int numBlk_a, int stride_int_t2,
...
// created by tc_gen_code_Kernel()
__global__ void kiaq_bcjq_abcijk_32x32x32x32x32x32x32_3(float *dev_t3, float *dev_t2, float *dev_v2,
                            int size_k, int size_j, int size_i, int size_c,
                            int size_b, int size_a, int size_q, int numBlk_k,
                            int numBlk_j, int numBlk_i, int numBlk_c,
                            int numBlk_b, int numBlk_a, int stride_int_t2,
...
#pragma unroll 1
  for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1) {
    ...
    // This Part is for Loading Input-Left
    // tc_gen_code_Kernel_Load_Inputs_Abstracts()
    if (0 < rng_i && idx_j < rng_k)
      for (int ll = 0; ll < rng_a; ll++) {
        // ['q', 'a', 'i', 'k']
        // Exception: Temp. version!: threadIdx.x + l
        // Exception: Temp. version!: 0 < rng_i
        sm_a[threadIdx.x][threadIdx.y + 0 + ll * 16] =
  ...
  // Part: Generalized Threads
  // Part: Generalized Register-Tiling
  if (idx_k < rng_k && idx_i < rng_i && idx_j < rng_j && idx_b < rng_b)
    for (int i = 0; i < 8; i++) {
      for (int j = 0; j < 4; j++) {
        if (i < rng_c && j < rng_a) {
          dev_t3[t3_base_thread + (i * stride_reg_y) + (j * stride_reg_x)] =
...
// created by tc_gen_code_Kernel()
__global__ void kiaq_bcjq_abcijk_32x32x32x32x32x32x32_4(float *dev_t3, float *dev_t2, float *dev_v2,
                            int size_k, int size_j, int size_i, int size_c,
                            int size_b, int size_a, int size_q, int numBlk_k,
                            int numBlk_j, int numBlk_i, int numBlk_c,
                            int numBlk_b, int numBlk_a, int stride_int_t2,
  ...
  for (int l = 0; l < size_internal; l += SIZE_INT_UNIT_1) {
    ...
    if (0 < rng_i && idx_j < rng_k &&
      ...
      for (int ll = 0; ll < rng_a; ll++) {
        ...
        // Exception: Temp. version!: threadIdx.x + l
        // Exception: Temp. version!: 0 < rng_i
        if (threadIdx.x + l < size_internal && 0 < rng_i)
          sm_a[threadIdx.x][threadIdx.y + 8 + ll * 16] =
              dev_t2[(blk_idx_a * SIZE_SLICE_1_A + ll +
                      (blk_idx_i * SIZE_SLICE_1_I + 0 +
                       (blk_idx_k * SIZE_SLICE_1_K + idx_j + 8) * size_i) *
...
void kiaq_bcjq_abcijk_32x32x32x32x32x32x32(int size_a, int size_b, int size_c, int size_i,
  ...
  int stride_reg_x_1 = stride_output_a;
  ...
  if (size_k % SIZE_SLICE_1_K == 0 && size_j % SIZE_SLICE_1_J == 0 &&
    ...
    if (size_q % SIZE_SLICE_1_Q == 0) {
      ...
      kiaq_bcjq_abcijk_32x32x32x32x32x32x32_1<<<gridsize_1, blocksize_1>>>(
      ...
      kiaq_bcjq_abcijk_32x32x32x32x32x32x32_2<<<gridsize_1, blocksize_1>>>(
          dev_t3, dev_t2, dev_v2, size_k, size_j, size_i, size_c, size_b,
          size_a, size_q, CEIL_DIV(size_k, SIZE_SLICE_1_K),
          CEIL_DIV(size_j, SIZE_SLICE_1_J), CEIL_DIV(size_i, SIZE_SLICE_1_I),
          CEIL_DIV(size_c, SIZE_SLICE_1_C), CEIL_DIV(size_b, SIZE_SLICE_1_B),
    ...
    if (size_q % SIZE_SLICE_1_Q == 0) {
      ...
      kiaq_bcjq_abcijk_32x32x32x32x32x32x32_3<<<gridsize_1, blocksize_1>>>(
          dev_t3, dev_t2, dev_v2, size_k, size_j, size_i, size_c, size_b,
          size_a, size_q, CEIL_DIV(size_k, SIZE_SLICE_1_K),
          CEIL_DIV(size_j, SIZE_SLICE_1_J), CEIL_DIV(size_i, SIZE_SLICE_1_I),
          CEIL_DIV(size_c, SIZE_SLICE_1_C), CEIL_DIV(size_b, SIZE_SLICE_1_B),
      ...
      kiaq_bcjq_abcijk_32x32x32x32x32x32x32_4<<<gridsize_1, blocksize_1>>>(
          dev_t3, dev_t2, dev_v2, size_k, size_j, size_i, size_c, size_b,
          size_a, size_q, CEIL_DIV(size_k, SIZE_SLICE_1_K),
          CEIL_DIV(size_j, SIZE_SLICE_1_J), CEIL_DIV(size_i, SIZE_SLICE_1_I),
          CEIL_DIV(size_c, SIZE_SLICE_1_C), CEIL_DIV(size_b, SIZE_SLICE_1_B),
